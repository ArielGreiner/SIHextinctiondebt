#based on the version of the code from 6.12.2016
      ...
      for(i in 1:length(dispV)){
        ...
        for(j in 1:3){ 
          ...
          for(TS in 1:Tmax){ #run through time steps
            ...
            #^ why, in the above line, do you divide by 1000?
            ...
            if(is.null(rownames(dispersal_matrix))){ 
              ...
              Rt0 <- DT*rInput+R0*(1-DT*(rLoss + sum(consume*N0)))} else { #resource step  
                #old code: envt.v<-envt.v[as.numeric(rownames(dispersal_matrix))]
                consume <- 0.1*(1.5-abs(sapply(eOptimum,'-',envt.v)))
                Nt <- N*(1+DT*(eff*R*consume - disp - mort)) + DT*Immigrants #abundance step
            ...
            if(TS == ((predel_collecttime*samplelength) + initial_time)){
              ...
              #old code: if(j==1){btw<-betweenness(weightedgraph)
              if(j==1){btw<-betweenness(graph_run)
              if(sum(btw)==0){
                patch.delete<-order(degree(graph_run),decreasing = T)[1:10]
              } else{patch.delete<-order(btw,decreasing = T)[1:10] }
              } else{
                if(j==2){patch.delete<-order(betweenness(graph_run),decreasing=F)[1:10]} else{
          ...
          #will have to change the 100's if change the # of communities
          L_SRlast.df<-gather(L_lastdebt,key = Patch,value=Debt_t,L_SR.V1:L_SR.V100) #wide -> long format
          L_loss2<-gather(L_loss,key = Patch, value=Loss, L_SR.V1:L_SR.V100)        
